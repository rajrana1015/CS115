...
def FA(x,y,cin):
    ...
    s=gor(gand(gnot(x),XOR(y,cin)),gand(x,gor(gand(gnot(y),gnot(cin)),gand(y,cin))))
    c=gor(gand(cin,XOR(x,y)),gand(gand(x,y),gor(gnot(cin),cin)))
    return (c,s)
...
def fourBitAdd(xxxx,yyyy):
    ...
    (e,zth) = FA(xxxx[1],yyyy[1],d)
    (f,zf) = FA(xxxx[0],yyyy[0],e)  
    return (f,(zf,zth,zt,zo))
...
def test_fourBitAdd():
    ...
    assert( ww == (0,1,0,1) and c == 0 )
    c,ww = fourBitAdd(one,one)
    assert( ww == (1,0,1,0) and c == 0 )
    c,ww = fourBitAdd(one,two)
    assert( ww == (1,1,1,0) and c == 0 )
    c,ww = fourBitAdd(three,three)
    assert( ww == (1,0,0,0) and c == 1 )
